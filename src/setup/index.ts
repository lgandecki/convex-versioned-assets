...
`);
  ...
  try {
    ...
    await step11GenerateInstructionsAndPrintNextSteps(state);
  } catch (error) {
...
async function step1PreflightChecks(state: SetupState): Promise<void> {
  ...
  if (!fileExists(apiTsPath) && !fileExists(apiDtsPath) && !fileExists(apiJsPath)) {
    throw new Error(
...
async function step2Dependencies(state: SetupState): Promise<void> {
  ...
  if (packagesToInstall.length > 0) {
    logWarning(`Missing packages: ${packagesToInstall.join(", ")}`);
    const install = await confirm(`Install ${packagesToInstall.join(" and ")} now?`);
    if (install) {
      const cmd = getInstallCommand(pm, packagesToInstall);
      log(`Running: ${cmd}`);
      if (!runCommand(cmd)) {
        throw new Error(`Failed to install packages: ${packagesToInstall.join(", ")}`);
      }
      logSuccess(`Installed ${packagesToInstall.join(", ")}`);
    } else {
      throw new Error(
        `Required packages not installed: ${packagesToInstall.join(", ")}. Please install them manually.`,
      );
    }
  }
}
...
async function step4CreateRequiredFiles(state: SetupState): Promise<void> {
  ...
  const generateUploadUrlPath = path.join(state.convexDir, "generateUploadUrl.ts");
...
async function step5UpdateConfigFiles(state: SetupState): Promise<void> {
  ...
  if (!fileExists(configPath)) {
    ...
    if (!content.includes("versionedAssets")) {
      ...
      logWarning("convex/convex.config.ts exists but doesn't include versionedAssets");
...
async function handleHttpTs(state: SetupState): Promise<void> {
  ...
  if (isDefaultHttpTemplate(content)) {
    logInfo("Detected default convex-auth http.ts template - auto-transforming...");
    const result = transformHttpTs(content);

    if (result.success && result.transformed) {
      writeFile(httpPath, result.transformed);
      logSuccess("Automatically updated convex/http.ts with versioned-assets routes");
      return;
    } else {
...
async function step8ConfigureTailwindCSS(state: SetupState): Promise<void> {
  ...
  if (!cssFilePath) {
    ...
    logInfo('  @source "../node_modules/convex-versioned-assets/dist/admin-ui/**/*.js";');
    return;
  }
  ...
  if (!content.includes('@import "tailwindcss"') && !content.includes("@import 'tailwindcss'")) {
    logWarning("CSS file doesn't appear to use Tailwind v4 (@import \"tailwindcss\")");
  ...
  logSuccess(`Added Tailwind @source directive to ${path.basename(cssFilePath)}`);
}
...
async function step10TransformAppTsx(state: SetupState): Promise<void> {
  ...
  if (!fileExists(appTsxPath)) {
    ...
    logSuccess("Created src/App.tsx with admin panel");
    return;
  }
  ...
  // Check if it's a fresh template
  const isFresh = isFreshAppTemplate(content);

  if (isFresh) {
    // Auto-replace fresh templates
    const result = replaceAppTsx(content);
    if (result.success && result.transformed) {
      writeFile(appTsxPath, result.transformed);
      logSuccess("Replaced App.tsx with admin panel setup");
      return;
    }
  }

  // Ask user before replacing customized App.tsx
  logWarning("App.tsx appears to have custom content.");
  const replace = await confirm(
    "Replace App.tsx with admin panel? (Your current App.tsx will be backed up)",
    false,
  );

  if (replace) {
    // Backup existing
    const backupPath = path.join(state.projectRoot, "src", "App.tsx.backup");
    writeFile(backupPath, content);
    logInfo(`Backed up existing App.tsx to ${backupPath}`);

    // Replace with admin panel
    writeFile(appTsxPath, appTsxTemplate);
    logSuccess("Replaced App.tsx with admin panel setup");
  } else {
    logWarning("Skipping App.tsx modification.");
    logInfo("Add AdminPanel component manually. See docs for example.");
  }
}

/**
 * Step 10: Generate instructions if needed and print next steps
 * Note: CSS is now imported from the library directly via:
 * import "convex-versioned-assets/admin-ui/styles";
 */
async function step11GenerateInstructionsAndPrintNextSteps(state: SetupState): Promise<void> {
  ...
  if (state.needsHttpManualUpdate || state.needsConfigManualUpdate) {
    ...
    const instructionsPath = path.join(state.projectRoot, "agents-instructions.md");
...
${colors.dim}For documentation, see: https://github.com/lgandecki/convex-versioned-assets${colors.reset}
`);
}
